...
namespace mastermindclone
{
    class mastermindclone
    {
        ...
        private static string PlayerInput; // what the player inputs in the console
        ...
        {           
            { 
                if (playgame == true && CodeNotguessed == true)
                {
                    while (Nvalid == false && Mvalid == false && CodeNotguessed == true)
                    {
                    ...
                    while (Mvalid == false && Nvalid == true && CodeNotguessed == true)
                    {
                ...
                {                   
                    ...
                    for (int i = 0; i < N; i++)
                    {
                        Console.WriteLine("Enter your guess for position {0}:", i); // asking the user to enter his guess for each position
                        inputval = 0; // input value set to 0 at the start
                        G = Console.ReadLine(); // reading the guess of the player from the console
                        Isitanum = int.TryParse(G, out inputval); // parsing the guess and checking if its an int using the boolean "Isitanum" 
